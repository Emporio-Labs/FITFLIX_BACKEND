...
const printFoodItems = (items: FoodItem[], diag: Diag, source: "plan" | "catalog") => {
...
const resolveMealIngredients = async (meal: Meal, diag: Diag): Promise<MacroRow> => {
  ...
  if (hasDirectItems) {
    console.log(`   ${yellow("▶")} Items from UserNutritionPlan snapshot (items[]):`);
  ...
  if (hasOptions) {
    ...
    if (!defaultOpt) {
      console.log(`   ${red("✗ options[] present but all entries are empty.")}`);
      return zeroMacros();
    }
    console.log(
  ...
  if (meal.recipeId) {
    ...
    if (ingredients.length === 0) {
      console.log(`     ${red("✗ No RecipeIngredient rows found in catalog either.")}`);
...
const traceTemplateSnapshot = async (plan: { sourceTemplateId?: unknown }) => {
  ...
  if (!tmpl) {
    console.log(red(`  ⚠ Source template ${plan.sourceTemplateId} NOT FOUND in DB.`));
    return;
  }
...
async function main() {
  ...
  diag.userFound = true;
  console.log(green(`✓ User found: ${user.username ?? "[no username]"} (${user._id})`));
  ...
  if (!plan) {
    ...
    if (allPlans.length === 0) {
      ...
      console.log(yellow(`⚠ No ACTIVE plan. Existing plans (${allPlans.length}):`));
      for (const p of allPlans) {
        console.log(dim(`   • ${p.name}  status=${p.status}  created=${p.createdAt}`));
      }
    }
    printDiag(diag);
    await mongoose.disconnect();
  ...
  if (days.length === 0) {
    ...
    console.log(
      yellow(`  This means the UserNutritionPlan was assigned from a template that had empty days[],\n`) +
  ...
  const [categoryCount, recipeCount, ingredientCount, foodCount] = await Promise.all([
  ...
  const nullFoodIdCount = await RecipeIngredient.countDocuments({ foodId: null });
...
function printDiag(diag: Diag) {
  ...
  console.log(`  Meals with empty items[]:       ${diag.mealsWithEmptyItems > 0 ? yellow(String(diag.mealsWithEmptyItems)) : "0"}`);
  console.log(`  Meals with options[]:           ${diag.mealsWithOptions}`);
  console.log();
  console.log(`  Total ingredient rows shown:    ${diag.totalItems}`);
  console.log(`    ↳ from plan snapshot (items): ${diag.itemsFromPlanSnapshot}`);
  console.log(`    ↳ from Recipe catalog fallback: ${diag.itemsFromRecipeCatalog}`);
  console.log(
    `  Missing foodId refs (items):    ` +
      (diag.missingFoodRefs > 0 ? red(String(diag.missingFoodRefs)) : green("0"))
  );
  console.log(
    `  Meals using catalog fallback:   ` +
      (diag.missingRecipeRefs > 0 ? yellow(String(diag.missingRecipeRefs)) : green("0"))
  );
  console.log(
  ...
  if (diag.unresolvedMeals.length > 0) {
    console.log();
    console.log(red(`  ✗ Fully unresolved meals (no items, options, or catalog data):`));
