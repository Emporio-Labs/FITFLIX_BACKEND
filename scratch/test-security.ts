...
async function runTests() {
	...
	if (!hmRes.ok) {
		throw new Error(`Health Markers failed: ${hmRes.status} - ${await hmRes.text()}`);
	}
	...
	if (!hgRes.ok) {
		throw new Error(`Health Goals failed: ${hgRes.status} - ${await hgRes.text()}`);
	}
	...
	const consentRes = await fetch(`${BASE_URL}/onboarding/consent`, {
		...
		body: JSON.stringify({
			consents: [
				{ type: "WELLNESS_SERVICES", accepted: true, signatureName: "Test User" },
				{ type: "GYM_FITNESS", accepted: true, signatureName: "Test User" },
			],
		}),
	});
	if (!consentRes.ok) {
		throw new Error(`Consent failed: ${consentRes.status} - ${await consentRes.text()}`);
	}
	...
	const dummyPdf = new Blob(["%PDF-1.4 mock pdf content"], { type: "application/pdf" });
	...
	if (returnedUrl.includes("s3.amazonaws.com") && !returnedUrl.includes("X-Amz-Expires")) {
		throw new Error("FAIL: Response returned a direct/unprotected S3 URL!");
	}
	if (!returnedUrl.includes("X-Amz-Expires=900") && !returnedUrl.includes("Expires=900")) {
		console.warn(`WARNING: Expiry token is present but not explicitly checking 900 seconds. URL is: ${returnedUrl}`);
	} else {
	...
	console.log("DB MedicalReport Document:", JSON.stringify(dbReport.toObject(), null, 2));

	if (dbReport.reportUrl) {
		if (dbReport.reportUrl.includes(".amazonaws.com") || dbReport.reportUrl.includes("fitflix-storage")) {
			throw new Error(`FAIL: MongoDB stored a direct S3 bucket URL in reportUrl: ${dbReport.reportUrl}`);
		}
	}
	...
	console.log(`PASS: MongoDB successfully stored the unique s3Key: ${dbReport.s3Key}`);

	// VERIFICATION 3: Fetch Onboarding Profile (Admin route) and check reports payload
	console.log(`Fetching onboarding profile for user ${userId} using Admin token...`);
	const profileRes = await fetch(`${BASE_URL}/users/${userId}/onboarding-profile`, {
		headers: {
	...
	console.log("Onboarding Profile Reports Payload:", JSON.stringify(profileJson.reports, null, 2));
	...
	if (profileReport.reportUrl.includes("s3.amazonaws.com") && !profileReport.reportUrl.includes("X-Amz-Expires")) {
		throw new Error("FAIL: Onboarding profile returned a direct unprotected S3 URL!");
	}
	console.log(`PASS: Onboarding profile returned dynamic pre-signed URL: ${profileReport.reportUrl}`);

	// VERIFICATION 4: Get Report Signed URL endpoint directly
	console.log(`Requesting signed URL for report ${reportId} directly...`);
	const signedUrlRes = await fetch(`${BASE_URL}/users/${userId}/reports/${reportId}/url`, {
		headers: {
			"Authorization": `Bearer ${adminToken}`
		}
	});
	...
	console.log("Signed URL Response Payload:", JSON.stringify(signedUrlJson, null, 2));

	if (signedUrlJson.expiresIn !== 900) {
		throw new Error(`FAIL: Signed URL endpoint expiresIn is ${signedUrlJson.expiresIn}, expected 900`);
	}
	if (!signedUrlJson.url.includes("X-Amz-Expires=900") && !signedUrlJson.url.includes("Expires=900")) {
		console.warn(`WARNING: Signed URL query parameter does not explicitly contain 900. URL: ${signedUrlJson.url}`);
	}
	console.log("PASS: Signed URL endpoint returned correct URL and 900s expiresIn.");
